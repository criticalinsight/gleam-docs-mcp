...
pub fn popular_packages_handler(_req: mcp.ReadResourceRequest) -> Result(mcp.ReadResourceResult, String) {
  case hex_client.search_packages("") {
    Ok(packages) -> {
      let text = packages 
        |> list.map(fn(p: hex_client.HexPackage) { p.name <> ": " <> p.description })
        |> string.join("\n")
        
      Ok(mcp.ReadResourceResult(
        meta: None,
        contents: [mcp.TextResource(mcp.TextResourceContents(
          uri: "gleam://packages",
