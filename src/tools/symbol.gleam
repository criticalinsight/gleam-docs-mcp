...
pub fn get_symbol_context(project_path: String, module_name: String, symbol_name: String) -> String {
  let module_file = string.replace(module_name, "/", "_") <> ".gleam"
  let src_path = filepath.join(project_path, "src") |> filepath.join(module_file)
  
  case simplifile.read(src_path) {
    Ok(content) -> {
      // Very simple greedy search for now
      case string.contains(content, "pub fn " <> symbol_name) {
        True -> "### Symbol Context: " <> module_name <> "." <> symbol_name <> "\n(Source found in " <> src_path <> ")"
...
pub fn get_remote_symbol(package_name: String, module: String, symbol: String) -> String {
  let url = "https://hexdocs.pm/" <> package_name <> "/" <> module <> ".html#" <> symbol
  "Remote URL: " <> url
}

pub fn get_symbol_context_handler(req: mcp.CallToolRequest(Dynamic)) -> Result(mcp.CallToolResult, String) {
